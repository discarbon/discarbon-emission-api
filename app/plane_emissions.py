...
def calculate_flight_distance(from_airport, to_airport):
    ...
    """
    from_location = AIRPORTS.query(f"iata_code == '{from_airport}'")[
        ["latitude_deg", "longitude_deg"]
    ]
    to_location = AIRPORTS.query(f"iata_code == '{to_airport}'")[["latitude_deg", "longitude_deg"]]

    from_coord = Coord(
        from_location.iloc[0]["latitude_deg"], from_location.iloc[0]["longitude_deg"]
    )
    to_coord = Coord(to_location.iloc[0]["latitude_deg"], to_location.iloc[0]["longitude_deg"])
...
def calculate_geodesic_distance(from_location, to_location):
    ...
    from_coord = Coord(math.radians(from_location.lat), math.radians(from_location.lon))
    to_coord = Coord(math.radians(to_location.lat), math.radians(to_location.lon))
